      ...
      if (res.ok) {
        ...
        if (user) {
          ...
          if (mine) {
            ...
            setFormInstagramUrl(mine.instagram_url ?? '');
            setFormTiktokUrl(mine.tiktok_url ?? '');
            setFormYoutubeUrl(mine.youtube_url ?? '');
            setFormFacebookUrl(mine.facebook_url ?? '');
            setFormXiaohongshuUrl(mine.xiaohongshu_url ?? '');
          }
        }
      }
    } catch { /* ignore */ }
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 mb-4">
              ...
              {myReview && !showReviewForm && (
                <div>
                  ...
                  <SocialIcons instagramUrl={myReview.instagram_url} tiktokUrl={myReview.tiktok_url} youtubeUrl={myReview.youtube_url} facebookUrl={myReview.facebook_url} xiaohongshuUrl={myReview.xiaohongshu_url} />
                    ...
                    <button
                      onClick={() => {
                        ...
                        if (myReview) {
                          ...
                          setFormInstagramUrl(myReview.instagram_url ?? '');
                          setFormTiktokUrl(myReview.tiktok_url ?? '');
                          setFormYoutubeUrl(myReview.youtube_url ?? '');
                          setFormFacebookUrl(myReview.facebook_url ?? '');
                          setFormXiaohongshuUrl(myReview.xiaohongshu_url ?? '');
                        }
                      }}
