...
const allRestaurants = diningData.restaurants as Restaurant[];
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Look up restaurant to get its ships
    const restaurant = allRestaurants.find(r => r.id === restaurantId);
    if (!restaurant) {
      return NextResponse.json({ sailings: [] });
    }

    // Fetch user's sailings on ships that have this restaurant
    ...
    // Check which sailings already have a dining review for this restaurant
