#!/usr/bin/env node
// Parse Disney Destiny itinerary text and merge into sailing-data.json

const fs = require('fs');
const path = require('path');

const rawText = fs.readFileSync(path.join(__dirname, 'destiny-itineraries.txt'), 'utf8');
...
for (let i = 0; i < lines.length; i++) {
  ...
  if (line === 'Current Disney Destiny Itineraries') {
    isPast = false;
    continue;
  }
  if (line === 'Past Disney Destiny Itineraries') {
    isPast = true;
    continue;
  }
...
const jsonEntries = unique.map(it => {
  let baseId = `destiny-${slugify(it.name)}`;
...
const nonDestiny = existing.itineraries.filter(it => !it.ships.includes('Disney Destiny') || it.ships.length > 1);
console.log(`Existing: ${existing.itineraries.length} itineraries, ${nonDestiny.length} non-Destiny`);

const merged = [...nonDestiny, ...jsonEntries];
