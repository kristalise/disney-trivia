#!/usr/bin/env node
// Parse Disney Treasure itinerary text and merge into sailing-data.json

const fs = require('fs');
const path = require('path');

const rawText = fs.readFileSync(path.join(__dirname, 'treasure-itineraries.txt'), 'utf8');
...
for (let i = 0; i < lines.length; i++) {
  ...
  if (line === 'Current Disney Treasure Itineraries') {
    isPast = false;
    continue;
  }
  if (line === 'Past Disney Treasure Itineraries') {
    isPast = true;
    continue;
  }
  ...
  const regionHeaders = [
    'Bahamian', 'Dark (No Passengers)', 'Eastern Caribbean',
    'New Build', 'Transatlantic', 'Western Caribbean',
  ];
...
const jsonEntries = unique.map(it => {
  let baseId = `treasure-${slugify(it.name)}`;
...
const nonTreasure = existing.itineraries.filter(it => !it.ships.includes('Disney Treasure') || it.ships.length > 1);
console.log(`Existing: ${existing.itineraries.length} itineraries, ${nonTreasure.length} non-Treasure`);

const merged = [...nonTreasure, ...jsonEntries];
