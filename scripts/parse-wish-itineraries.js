#!/usr/bin/env node
// Parse Disney Wish itinerary text and merge into sailing-data.json

const fs = require('fs');
const path = require('path');

const rawText = fs.readFileSync(path.join(__dirname, 'wish-itineraries.txt'), 'utf8');
...
for (let i = 0; i < lines.length; i++) {
  ...
  if (line === 'Current Disney Wish Itineraries') {
    isPast = false;
    continue;
  }
  if (line === 'Past Disney Wish Itineraries') {
    isPast = true;
    continue;
  }
  ...
  if (nextLine && nextLine.includes(' • ')) {
    ...
    if (name.includes('Dry Dock') || name.includes('No Passengers') ||
        ...
        name.includes('Delivery Placeholder') || name.includes('Dark (No Passengers)')) {
      i++;
      continue;
    }
  ...
  const regionHeaders = [
    'Bahamian', 'Belgium', 'British Isles', 'Dark (No Passengers)',
    'Dry Dock', 'Greek Isles', 'Mediterranean', 'Netherlands',
    'New Build', 'Northern European', 'Norwegian Fjords', 'Spain',
    'Transatlantic', 'Western Caribbean', 'Western Europe',
  ];
...
const jsonEntries = unique.map(it => {
  let baseId = `wish-${slugify(it.name)}`;
...
const nonWish = existing.itineraries.filter(it => !it.ships.includes('Disney Wish') || it.ships.length > 1);
console.log(`Existing: ${existing.itineraries.length} itineraries, ${nonWish.length} non-Wish`);

const merged = [...nonWish, ...jsonEntries];
