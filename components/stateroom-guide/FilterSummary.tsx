...
export default function FilterSummary({
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow-lg border border-slate-200 dark:border-slate-700 mb-4">
      ...
      {/* Advisory messages based on result breadth */}
      {totalShipRooms > 0 && filteredCount > totalShipRooms * 0.5 && (
        <p className="mt-3 text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1.5">
          ...
          Your results are very broad. Add more preferences to narrow down your options.
        </p>
      )}
      {totalShipRooms > 0 && filteredCount > 0 && filteredCount < totalShipRooms * 0.1 && (
        <p className="mt-3 text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1.5">
          <svg className="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          Your results are very narrow. Remove some criteria to widen the search.
        </p>
      )}
    </div>
  );
